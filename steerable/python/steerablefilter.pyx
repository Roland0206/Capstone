...
cdef class Detector2D:
    ...
    def __cinit__(self, np.ndarray[double, ndim=2, mode='c'] pixels, int order, double sigma, int border_condition=3):
        ...
        self.thisptr = new SteerableDetector(&pixels[0,0], pixels.shape[1], pixels.shape[0], order, sigma, border_condition)
    ...
    def filter(self):
        ...
        dims[1] = self.thisptr.nx_
        dims[0] = self.thisptr.ny_
